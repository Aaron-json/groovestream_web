        ...
        {(field) => {
          const handleFiles = (newFiles: File[]) => {
            ...
            for (const file of newFiles) {
              if (field.state.value.length + acceptedFiles.length >= MAX_FILES) {
          ...
          return field.state.value.length === 0 ? (
...
function FilesList({ files, onRemove, onAddMore }: FilesListProps) {
  const fileInputRef = useRef<HTMLInputElement>(null);
